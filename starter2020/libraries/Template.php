...
class Template {
    ...
    protected $brand_name = COMPANY." - ".APPSNAME; 
    ...
    protected $header_assets = array();
    protected $footer_assets = array();
    ...
    public function add_header_assets($src,$type=FALSE)
    {
        // $this->_ci->load->helper('url');
        // $this->js['js']=$src;
        $this->header_assets[]=array('src'=>$src,'type'=>$type);

          
    }
    public function add_footer_assets($src,$type=FALSE)
    {
        // $this->_ci->load->helper('url');
        // $this->js['js']=$src;
        $this->footer_assets[]=array('src'=>$src,'type'=>$type);
    ...
    public function render($view, $data = array(), $return = FALSE){
        ...
        */
        ...
        $header_assets = array();
        foreach ($this->header_assets as $header_assets_item){
            $type=$header_assets_item['type'];
            $src=$header_assets_item['src'];
            ...
            switch ($type){
                ...
                case 'css':
                    ...
                    break;
                case 'js_embed':
                    $srcx = '<script type="javascript">'.$src.'</script>';
                    $footer_assets[]=$srcx;
                    break;
                case 'css_embed':
                    $srcx = '<style>'.$src.'</style>';
                    $footer_assets[]=$srcx;
                    break;
        
                }
        }
        $header_assets = implode('', $header_assets);

        $footer_assets = array();
        foreach ($this->footer_assets as $footer_assets_item){
            $type=$footer_assets_item['type'];
            $src=$footer_assets_item['src'];
        ...
        $footer_assets = implode('', $footer_assets);


        
        //flexible header and footer view
        $head = $this->_ci->load->view('themes/'.$this->theme.'/'.$this->header, array(), TRUE);
        // $head = $this->_ci->load->view('themes/'.$this->theme.'/header', array(), TRUE);
        $foot = $this->_ci->load->view('themes/'.$this->theme.'/'.$this->footer, array(), TRUE);
        // $foot = $this->_ci->load->view('themes/'.$this->theme.'/footer', array(), TRUE);
        $main_content = $this->_ci->load->view($view, $data, TRUE);
       /*  if($this->theme!='default'):
            $body = $this->_ci->load->view($this->theme.'/layout/' . $this->layout, array(
                'header' => $header,
                'footer' => $footer,
                'main_content' => $main_content,
            ), TRUE);
        else: */
            $body = $this->_ci->load->view('themes/'.$this->theme.'/layout/' . $this->layout, array(
                'header' => $head,
                'footer' => $foot,
        ...
        return $this->_ci->load->view('themes/'.$this->theme.'/'.$this->baseview, array(
            'title' => $title,
