...
class Auth extends CI_Controller
{
	...
	public function __construct()
	{
		parent::__construct();
		$this->load->database();
		// $this->load->library(array('ion_auth', 'form_validation','session','google'));
		...
		$this->temp->add_js('var baseurl="'.base_url('auth').'";','embed');
	}
	/*Updated with Google Auth*/
	...
	function coba(){
		...
		$this->temp->render('welcome_message');
	}
	...
	public function get_users(){
		$this->data['title'] = $this->lang->line('index_heading');
			...
			$this->temp->add_header_assets(themes_url('custom/datatables/').'/datatables.bundle.css','css');
        	$this->temp->add_footer_assets(themes_url('custom/datatables/').'/datatables.bundle.js','js');
	...
	public function login()
	{
		...
		if ($this->form_validation->run() === TRUE)
		{
			...
			if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember))
			{
                ...
                echo (json_encode(array("status"=>"success","url"=>$redirect,"msg"=>strip_tags($this->ion_auth->messages()))));
			...
			else
			{
				...
				echo json_encode(array("status"=>"failed","msg"=>strip_tags($this->ion_auth->errors())));
...
*/
			...
			$this->temp->render('auth/login', $this->data);
	...
	*/
	...
	public function _render_page($view, $data = NULL, $returnhtml = FALSE)//I think this makes more sense
	{
		...
		$view_html = $this->temp->load_view($view, $viewdata, false);
		// $view_html = $this->temp->render($view, $this->viewdata, $returnhtml);
